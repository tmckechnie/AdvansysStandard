...
def GetCurrentCalendarMonths(Database,CalendarID,PeriodStartUTC,Refresh=False):
	...
	if PeriodStartUTC is None:
		PeriodStartUTC = system.date.addYears(system.date.now(),-3)
	params = {}
...
#---------------------------------
def GetTimePeriodsToCreated(Database,MergedCalendarData,CalendarName,FutureIntervalsInMonths=1,MinuteInterval=30):
...
def CreateTimePeriods(TimePeriodsToCreated):
	...
	Standard.InterfaceModules.Flow.Common.Logmessage(LoggerName='CreateTimePeriods',Message="Time Periods to Create: " + str(TimePeriodsToCreated),DebugLevel=10)
	...
	def create(intervals):
		...
		for period in intervals:
			#return params
			params = {}
			params["database"] = str(period["database"])
		...
		return results
	Standard.InterfaceModules.Flow.Common.Logmessage(LoggerName='CreateTimePeriods',Message='Results to be created!',DebugLevel=10)	
	results = {}
	results['minutes'] = create(minutes)
	Standard.InterfaceModules.Flow.Common.Logmessage(LoggerName='CreateTimePeriods',Message='Minutes created!',DebugLevel=10)	
	results['hours'] = create(hours)
	Standard.InterfaceModules.Flow.Common.Logmessage(LoggerName='CreateTimePeriods',Message='hours created!',DebugLevel=10)	
	results['days'] = create(days)
	Standard.InterfaceModules.Flow.Common.Logmessage(LoggerName='CreateTimePeriods',Message='days created!',DebugLevel=10)	
	results['months'] = create(months)
	Standard.InterfaceModules.Flow.Common.Logmessage(LoggerName='CreateTimePeriods',Message='months created!',DebugLevel=10)	
	Standard.InterfaceModules.Flow.Common.Logmessage(LoggerName='CreateTimePeriods',Message='results: ' + str(results),DebugLevel=10)
	Standard.InterfaceModules.Flow.Common.Logmessage(LoggerName='CreateTimePeriods',Message='End!',DebugLevel=10)
	#return the year of the final end period. Needed to update the year tag when it rolls over to the next year's 445 pattern.
	#year period is then added into Flow when the year tag updates. THis is on the year's change script
	finalPeriodEndYear = None
	if len(results["months"]) == 0:
		finalPeriodEndMonth = system.date.parse(toBeCreated['months']["pe"])
		finalPeriodEndYear = system.date.getYear(finalPeriodEndMonth)
	return finalPeriodEndYear
	
#====================================================== 	GetCurrentYearTimePeriodID  ====================================================
def GetCurrentYearTimePeriod(database,calendarID,year):
	#Year was just added so the tag refreshes when the year rolls over
	params = {
			  "database":database,
			  "CalendarID":calendarID
				}
	resultDataset = system.db.runNamedQuery("Flow/Calendar Manager/GetCurrentYearTimePeriod",parameters = params)
	#return TimePeriodID and PeriodEnd of current year in UTC
	timePeriodID = resultDataset.getValueAt(0,"TimePeriodID")
	periodEnd = resultDataset.getValueAt(0,"PeriodEnd")
	#Update Period End to Local Time
	periodEndLocal = Standard.InterfaceModules.Flow.PrivateAPI.V1.Common.GetLocalTimeFromFlowUTCFormatted(periodEnd,Format = "yyyy-MM-dd HH:mm:ss")
	periodEndLocal = system.date.format(periodEndLocal,'yyyy-MM-dd HH:mm:ss')
	resultObject = {
			"TimePeriodID":timePeriodID,
			"PeriodEnd":periodEndLocal	
				}	
	resultJson = system.util.jsonEncode(resultObject)
	return resultJson
#====================================================== 	UpdateCurrentYearStartOfDay  ====================================================

def UpdateCurrentYearStartOfDay(database,timePeriodID,periodEndLocal):
	periodEndUTC = Standard.InterfaceModules.Flow.PrivateAPI.V1.Common.GetUTCTime(LocalTimestamp=periodEndLocal)
	params = {
		  "database":database,
		  "TimePeriodID":timePeriodID,
		  "PeriodEnd":periodEndUTC
			}
	result = system.db.runNamedQuery("Flow/Calendar Manager/UpdateCurrentYearPeriodEnd",parameters = params)
	return result
...
def GetStartofDay(SiteTagPath):
	...
	return startOfDay
#====================================================== 	getStartofDayforDateTimeLocal  ====================================================
def getStartofDayforDateTimeLocal(SiteTagPath,localDateTime):
	#Get current Calendar Months from tag
	currentPeriodsTagPath = SiteTagPath + "Calendar Manager/CurrentCalendarPeriods['periods']"
	currentPeriods = Standard.Model.Common.TagRead(currentPeriodsTagPath)
	if currentPeriods is not None:
		currentPeriods.reverse() # Start closest to now since chances are it will be closest to now
		for period in currentPeriods:
			periodStartLocal = system.date.parse(period["PeriodStart"])
			periodEndLocal = system.date.parse(period["PeriodEnd"])
			#If in Calendar Periods then get start of day for that calendar period
			if system.date.isBetween(localDateTime,periodStartLocal,periodEndLocal):
				startOfDay = system.date.getHour24(periodStartLocal)
				#Update local time to use that start of day instead
				localDateTime = system.date.setTime(localDateTime,startOfDay,0,0)
				return localDateTime	
	#If not found then default to current local time hour	
	return localDateTime
